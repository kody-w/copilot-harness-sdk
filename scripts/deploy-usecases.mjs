...
if (/<org>|<environment-id>|YourSolution/.test(`${cfg.environmentUrl}${cfg.environmentId}${cfg.shared?.environmentMcp?.connectionReference}`) || cfg.usecases.some((u) => /<your-/.test(`${u.domainMcp?.connectionReference || ''}${u.domainMcp?.connectorId || ''}`))) { console.error('usecases.json holds placeholders. Put your environmentUrl, environmentId, the shared connection reference and each use case\'s domainMcp connectionReference/connectorId in usecases/usecases.local.json (see usecases/README.md).'); process.exit(2); }
